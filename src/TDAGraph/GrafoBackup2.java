...
public class GrafoBackup2<V,E> implements Graph<V,E> {
...
//Vertice y Arco
	...
	private void DFSVertexBoole(GrafoBackup2<V,E> g, Vertex<V> v) {
		VerticeInterno<V> ver = (VerticeInterno<V>) v;
	...
	private void DFSMapeo(GrafoBackup2<V,E> g, Vertex<V> v, Map<Vertex<V>, Boolean> m) {
		try {
			m.put(v, true);
		} catch (InvalidKeyException e) {
			e.printStackTrace();
	...
	private void BFS(GrafoBackup2<V,E> g, Vertex<V> v) {
		...
		while(!cola.isEmpty()) {
			try {
				...
				VerticeInterno<V> vert= (GrafoBackup2<V, E>.VerticeInterno<V>) ent.getKey();
	...
	public boolean BFScaminoMasCorto(Vertex<V> alpha, Vertex<V> omega) {
		for(Vertex<V> v:this.vertices) {
			...
			castOff.setVisitado(false);
		}
		return this.BFSShortestPath(this, alpha, omega);
	}
	
	private boolean BFSShortestPath(GrafoBackup2<V,E> g, Vertex<V> start, Vertex<V> end) {
