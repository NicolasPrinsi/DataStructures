...
public class HeapPQueue<K,V> implements PriorityQueue<K,V> {
	...
	public HeapPQueue(Comparador<K> comp) {
		size=0;
		this.comp=comp;
		elems =(Entrada<K,V>[]) new Entrada[100];
	}
	@Override
	public int size() {
		return size;
	...
	public Entry removeMin() throws EmptyPriorityQueueException {
		...
		while(pos<=size&&!reached) {
			int izq=pos*2;
			int der=(pos*2)+1;
			if(izq<elems.length&&elems[izq]!=null) {
				if(der<elems.length&&elems[der]!=null) {
				...
				} else if(this.esMenor(elems[izq], elems[pos])) {
					this.theOldSwitcharoo(izq, pos);
					pos=izq;
				}else reached=true;
			} else reached=true;
